...
User=%{name}
Group=%{name}
ExecStart=%{_datadir}/%{name}/api-gateway.sh -c /%{confdir}/proxies.xml
Restart=always
RestartSec=10
...
%{__rm} -f %{buildroot}%{homedir}/api-gateway.bat
...
sed -i 's#CLASSPATH="$MEMBRANE_HOME/conf"#CLASSPATH="/etc/membrane"#' %{buildroot}%{homedir}/api-gateway.sh
