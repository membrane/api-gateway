...
package com.predic8.membrane.tutorials.oauth2;
...
/**
 * Two-instance tutorial (50a + 50b): 50a issues signed JWTs and publishes its public
 * keys at a JWKS endpoint, 50b validates those tokens by fetching the keys over HTTP.
...
public class OAuth2DistributedValidationTutorialTest extends DistributionExtractingTestcase {
    ...
    void startInstances() throws Exception {
        issuer = new Process2.Builder().in(baseDir).script("membrane")
                .withParameters("-c 50a-OAuth2-Distributed-Issuer.yaml").waitForMembrane().start();
        validator = new Process2.Builder().in(baseDir).script("membrane")
                .withParameters("-c 50b-OAuth2-Distributed-Validation.yaml").waitForMembrane().start();
    }
