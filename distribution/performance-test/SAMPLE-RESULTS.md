...
All three runs completed with **0 errors** out of 1,000,000 requests each, at concurrency 175.
Scenarios 1 and 3 use the same fixed 33-byte JSON body; scenario 2 uses a more realistic ~1 KB
JSON body (see "How this run was executed" below) -- so, unlike earlier rounds of this test, the
three numbers are not a strict apples-to-apples comparison against each other, only against their
own prior same-body measurements in TESTED-CONFIGURATIONS.md.
...
- The backend in scenarios 2 and 3 is a trivial echo (`java/LoadTesterBackend.java`), not a real
  service with its own business logic or database -- it isolates gateway-side cost specifically,
  which is the point of this test, but it is not a realistic backend latency profile.
