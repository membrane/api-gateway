...
<serviceProxy name="Membrane Resource service" port="2001">
    ...
    <groovy>
        ...
        exc.request.getHeader().add('Authorization',oauth2.userinfo.email)
        CONTINUE
    </groovy>
...
When the OAuth2/OpenID-Connect flow is done everything of importance is collected on the outgoing exchange as a property called "oauth2".
This property can be used to customize the call to the target e.g. to set the authorization header. In this example we use the groovy interceptor
to put the email address of the user ( that was requested through the profile scope ) as the value of the authorization header.

The "oauth2" property on the exchange is of type OAuth2AnswerParameters and has several properties that can be accessed.
The accessToken property is the access token that is generated by the token endpoint.
The tokenType property is the token type of the given access token.
The idToken property is the id token that is generated by the token endpoint in openid scope.
The userinfo property is a map with all the requested claims.
