...
3. **Code quality.** Match the codebase's own idioms in the changed files — Java 21, `var`, no
   needless intermediate variables, text blocks, `formatted` over concatenation, early returns,
   braces — plus `docs/CONVENTIONS.md` (don't abbreviate parameter names in public interfaces;
   private methods are fine). Flag only real deviations in the changed code, not pre-existing
   style elsewhere.
...
Map the changed files to a module from their path (`core/src/...` → `core`), then run just the
class(es)/package(s) the branch touched — never the whole suite without asking the user first
(see the `core` gotcha below):

```bash
# core: isolate the touched class or package (see gotcha below for why -Dtest alone won't do it)
test/scripts/run-core-test.sh com.predic8.membrane.core.some.TouchedTest

# other modules: no suite-isolation trap, so -Dtest works directly
mvn -pl annot test -Dtest=TouchedTest -Duser.language=en -Duser.country=US
```
...
- **`core`: `-Dtest=SomeClass` does not isolate a single class.** `core`'s `UnitTests` is a JUnit
  Platform `@Suite` that runs the whole package regardless of the `-Dtest` filter. Use
  `test/scripts/run-core-test.sh <FQCN or package>` to isolate the class(es)/package(s) the
  branch actually touched instead of running the whole `core` suite — the full suite is slow and
  can fail on unrelated/network-dependent tests offline. Ask the user first if you think a full
  module run is warranted.
