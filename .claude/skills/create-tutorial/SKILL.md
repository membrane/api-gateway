...
In this repo a tutorial is usually a **runnable, self-teaching YAML config**.
...
- **Pin the schema line** to whatever sibling files use (grep one: `grep -m1 schema=
  ...
  invent a number.
- **Keep the comment block short.** Steps are a command and its expected output — nothing more.
...
- `user()` — authenticated username (from `basicAuthentication` security scheme)
- `scopes()` — list of granted scopes
- `hasScope("read")` — boolean scope check
...
- [ ] `.md` steps have no integration test
