...
public class WebServerInterceptor extends AbstractInterceptor {
    ...
    private @NotNull Outcome handleRequestInternal(Exchange exc) throws IOException {
        ...
        if (generateIndex) {
            if (router.getResolverMap().getChildren(combine(getBeanBaseLocation(), docBase, uri)) != null) {
        ...
        try {
            ...
            exc.setResponse(createResponseInternal(router.getResolverMap(), combine(getBeanBaseLocation(), docBase, uri)));
            exc.setReceived();
    ...
    private boolean tryToReceiveResource(Exchange exc, String uri) {
        for (String i : index) {
            try {
                Response response = createResponseInternal(router.getResolverMap(), ResolverMap.combine(getBeanBaseLocation(), docBase, uri + i));
                if (!response.isOk())
                    continue;
    ...
    private Outcome generateHtmlResponseFromChildren(Exchange exc, String uri) throws FileNotFoundException {
        List<String> children = router.getResolverMap().getChildren(combine(getBeanBaseLocation(), docBase, uri));
        if (children == null) {
            return null;
        }
    ...
    private @NotNull String getNewPath(String path) {
        String newPath = combine(getBeanBaseLocation(), path);
