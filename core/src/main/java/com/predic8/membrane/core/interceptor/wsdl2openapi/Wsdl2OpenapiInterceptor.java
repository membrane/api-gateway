...
public class Wsdl2OpenapiInterceptor extends AbstractInterceptor {
    ...
    /**
     * The SOAP response as a parser input. See {@link XMLInputSourceUtil#getInputSource(InputStream, String)}
     * for how the Content-Type charset and a byte order mark are reconciled.
     */
    private static InputSource soapSource(Response response, InputStream body) {
        return XMLInputSourceUtil.getInputSource(body, response.getHeader().getCharset());
    }
