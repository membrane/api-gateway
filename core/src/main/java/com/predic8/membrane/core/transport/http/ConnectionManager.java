...
public class ConnectionManager {
    ...
    public Connection getConnection(String host, int port, String localHost, SSLProvider sslProvider, int connectTimeout, @Nullable String sniServerName,
        ...
        log.debug("connection requested for {}:{}{}", host, port, getProxyDescription(proxy));
        ...
        synchronized (this) {
            List<OldConnection> l = availableConnections.get(key);
            if (l != null) {
                int i = l.size() - 1;
                while (i >= 0) {
                    ...
                    Collections.swap(l, 0, i); // TODO ? Explain
                    i--;
                }
            }
        }
    ...
    private static @NotNull String getProxyDescription(@org.jetbrains.annotations.Nullable ProxyConfiguration proxy) {
        return proxy != null ? " via %s:%d".formatted(proxy.getHost(), proxy.getPort()) : "";
    }
            ...
            for (Map.Entry<ConnectionKey, ArrayList<OldConnection>> e : availableConnections.entrySet()) {
                ...
                for (int i = 0; i < l.size(); i++) {
                    ...
                    if (o.deathTime < now) {
                        ...
                        // TODO Needed? l.remove shifts the remaining entries
                        // replace [i] by [last]
                        if (i == l.size() - 1)
                            l.remove(i);
                        else
    ...
    /**
     * TODO Should numberInPool adjust here too?
     */
    private void close(List<Connection> connections) {
        ...
        log.debug("closed {} connections", connections.size());
    }
