 ...
 *
 * <p>The {@code id} member may be a {@link String}, an Integer {@link Integer}, or {@code null}.
...
@JsonSerialize(using = JSONRPCRequest.Serializer.class)
public class JSONRPCRequest {
    ...
    /** May be String, integral Number, or null. */
    private Object id;
    ...
    private static JSONRPCRequest fromNode(JsonNode root) throws IOException {
        ...
        req.jsonrpc = parseVersion(root, "request");

        JsonNode methodNode = root.get("method");
        if (methodNode == null || !methodNode.isTextual()) throw new IOException("Invalid JSON-RPC request: 'method' must be a string");

        try {
            req.setMethod(methodNode.asText());
        } catch (IllegalArgumentException e) {
            throw new IOException("Invalid JSON-RPC request: " + e.getMessage(), e);
        }

        if (root.has("id")) {
            req.setId(JSONRPCUtil.parseId(root.get("id"), "request"), true);
        }

        JsonNode paramsNode = root.get("params");
        if (paramsNode != null && !paramsNode.isNull()) {
            if (paramsNode.isArray()) {
                req.setParamsList(OM.convertValue(paramsNode, LIST_TYPE));
    ...
    public void setId(Object id, boolean present) {
        ...
        this.id = present ? normalizeId(id) : null;
    }
    ...
    private void validate() {
        ...
        if (paramsMap != null && paramsList != null) {
            throw new IllegalStateException("paramsMap and paramsList are mutually exclusive");
        }
        if (idPresent) {
            id = normalizeId(id);
        } else if (id != null) {
            throw new IllegalStateException("id must be null when id is absent");
        }
    }
    ...
    static final class Serializer extends StdSerializer<JSONRPCRequest> {
