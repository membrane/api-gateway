...
public class SetBodyInterceptor extends AbstractExchangeExpressionInterceptor {
    ...
    /**
     * @description Body content to set, evaluated as a SpEL template expression by default.
     * The result is encoded as UTF-8 bytes.
     * @example Name: ${header.X-Name}
     */
    @MCAttribute
    ...
    /**
     * @description Content-Type applied to the message after the body is written.
     * When set to a JSON media type, expression values are automatically escaped to prevent injection.
     * When omitted, no escaping is applied.
