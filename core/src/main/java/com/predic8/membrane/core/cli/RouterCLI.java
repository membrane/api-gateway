...
public class RouterCLI {
    ...
    private static Router initRouterByYAML(String location) throws Exception {
        ...
        BeanRegistry registry = new BeanRegistryImplementation(router, new GrammarAutoGenerated());
        registry.registerBeanDefinitions(parseMembraneResources(router.getResolverMap().resolve(location), new GrammarAutoGenerated()));
        registry.start();
