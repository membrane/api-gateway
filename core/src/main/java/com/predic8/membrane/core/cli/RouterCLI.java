...
public class RouterCLI {
    ...
    private static Router initRouterByYAML(String location) throws Exception {
        ...
        var bds = reg.parseYamlBeanDefinitions(router.getResolverMap().resolve(location), grammar);

        bds.stream()
                .filter(bd -> "configuration".equals(bd.getKind()))
                .findFirst()
                .ifPresent(bd -> {
                    Configuration cfg = (Configuration) reg.resolve(bd.getName());
                    router.applyConfiguration(cfg);
                });
