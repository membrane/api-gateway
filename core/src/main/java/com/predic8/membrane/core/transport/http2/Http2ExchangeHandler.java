...
public class Http2ExchangeHandler implements Runnable {
    ...
    private void invokeHandlers() throws AbortException {
        try {
            getFlowController().invokeRequestHandlers(exchange, transport.getInterceptors());
            ...
            switch (e) {
                case AbortException abortException ->
                        throw abortException; // TODO: migrate catch logic into this method
    ...
    private FlowController getFlowController() {
        return transport.getRouter().getFlowController();
    }
