...
import com.predic8.membrane.shaded.io.swagger.v3.oas.models.servers.Server;
...
public class APIProxy extends ServiceProxy implements Polyglot, XMLSupport {
    ...
    private Map<String, OpenAPIRecord> getOpenAPIMap() {
        ...
        apiRecords.forEach((id, rec) -> rec.api.getServers().forEach(server -> {
            ...
            try {
                paths.put(ensureTrailingSlash(getPathFromURL(router.getConfiguration().getUriFactory(), url)), rec);
            } catch (URISyntaxException e) {
    ...
    @MCElement(name = "description", component = false, mixed = true, collapsed = true)
