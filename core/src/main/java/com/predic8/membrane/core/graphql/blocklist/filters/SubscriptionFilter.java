...
@MCElement(name = "subscription")
public class SubscriptionFilter implements GraphQLFeatureFilter {

    @Override
    public void filter(ExecutableDocument ed) throws GraphQLOverHttpValidationException {
        ed.getOperationDefinitions().forEach(op -> {
            if ("subscription".equals(op.getOperationType().getOperation())) {
                throw new GraphQLOverHttpValidationException("Subscriptions are not permitted.");
            }
        });
    }
}