...
public abstract class AbstractJsonRPCMethodSchemas {
    ...
    /**
     * @description
     * <p>Defines a map from method name to JSON Schema location.</p>
     *
     * <p>The keys are matched against the JSON-RPC <code>method</code> value.</p>
     *
    ...
    public boolean isEmpty() {
        return mappings.isEmpty() && paramMappings.isEmpty();
    }
    ...
    public void init(Resolver resolver, URIFactory uriFactory, String beanBaseLocation) {
        ...
        if (resolver == null || uriFactory == null) {
            throw new ConfigurationException("Cannot initialize JSON-RPC schemas without resolver context.");
        }
        ...
        for (Param entry : effectiveMappings) {
            ...
            if (resolvedSchemas.containsKey(methodName)) {
                throw new ConfigurationException("Duplicate JSON-RPC schema mapping for method '%s'.".formatted(methodName));
            }
            resolvedSchemas.put(methodName,
                    loadSchema(methodName, entry.getSchema(), resolver, uriFactory, beanBaseLocation));
        }
        schemas = Collections.unmodifiableMap(resolvedSchemas);
    }
    ...
    private static Schema loadSchema(String methodName, String schemaPath, Resolver resolver, URIFactory uriFactory, String beanBaseLocation) {
        if (schemaPath == null || schemaPath.trim().isEmpty()) {
            throw new ConfigurationException("JSON-RPC schema path for method '%s' must not be empty.".formatted(methodName));
        }
        ...
        try (var in = resolver.resolve(resolvedLocation)) {
            ...
            throw new ConfigurationException("Cannot read JSON-RPC schema for method '%s' from '%s'.".formatted(methodName, schemaPath), e);
        } catch (RuntimeException e) {
            throw new ConfigurationException("Cannot create JSON-RPC schema for method '%s' from '%s'.".formatted(methodName, schemaPath), e);
        }
    }
    ...
    private List<Param> getEffectiveMappings() {
        if (!mappings.isEmpty() && !paramMappings.isEmpty()) {
            throw new ConfigurationException("Configure JSON-RPC schemas either as a YAML map or as XML <param> child elements, not both.");
        }
    ...
    public static class Param {
        ...
        /**
         * @description The exact JSON-RPC <code>method</code> value whose payload should be validated.
         * @example rpc.echo
         */
        @Required
        @MCAttribute
        ...
        /**
         * @description The path or URL of the JSON Schema used for the configured method.
