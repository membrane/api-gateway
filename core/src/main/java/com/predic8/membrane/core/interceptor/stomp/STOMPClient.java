...
public class STOMPClient extends AbstractInterceptor {
	...
	public void init() {
		...
		if (sslOutboundParser != null)
			sslOutboundProvider = new StaticSSLContext(sslOutboundParser, router.getResolverMap(), getBeanBaseLocation());
	}
	...
	public Outcome handleRequestInternal(Exchange exc) throws IOException {
		...
		if (isStomp1_0 || isStomp1_1orAbove) {
			...
			StreamPump.setupConnectionForwarding(exc, c, "STOMP", getRouter().getStatistics().getStreamPumpStats());
		} else {
