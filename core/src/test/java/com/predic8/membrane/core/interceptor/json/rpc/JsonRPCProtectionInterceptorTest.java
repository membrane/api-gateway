...
public class JsonRPCProtectionInterceptorTest {
    ...
    @Test
    void errorValidationSupportsInlineSchema() throws Exception {
        JsonRPCErrorValidation errorValidation = new JsonRPCErrorValidation();
        errorValidation.setSchema(inlineSchema(Map.of(
                "type", "object",
    ...
    @Test
    void schemaValidationSupportsMethodsWithLocationAndInlineSchema() throws Exception {
        JsonNode node = YAML.readTree("""
                jsonRPCProtection:
                  schemaValidation:
                    error:
                      location: error.json
                    methods:
                      'rpc.echo':
                        params:
                          location: echo-params.schema.json
                        response:
                          schema:
                            type: object
                            properties:
        ...
        var interceptor = GenericYamlParser.createAndPopulateNode(
        ...
        assertEquals("error.json", schemaValidation.getErrorValidation().getLocation());

        Map<String, JsonRPCSchemas> methods = schemaValidation.getMethods().getMethods();
        assertEquals("echo-params.schema.json", methods.get("rpc.echo").getParams().getLocation());
        assertEquals("echo-params.schema.json", methods.get("get").getParams().getLocation());

        Map<String, Object> responseSchema = methods.get("rpc.echo").getResponse().getSchema().getProperties();
        assertEquals("object", responseSchema.get("type"));
        assertEquals(
                "string",
                ((Map<?, ?>) ((Map<?, ?>) responseSchema.get("properties")).get("foo")).get("type")
        );
    }

    @Test
    void schemaValidationSupportsInlineErrorSchema() throws Exception {
        JsonNode node = YAML.readTree("""
                jsonRPCProtection:
                  schemaValidation:
                    error:
        ...
        assertNull(schemaValidation.getErrorValidation().getLocation());
        assertEquals(
                "object",
