...
class URIVsJavaNetURITest {
    ...
    private static String expectedPathWithQueryFromJava(java.net.URI j) {
        String rawPath = j.getRawPath();
        if (rawPath == null || rawPath.isBlank()) rawPath = "/";
        String rawQuery = j.getRawQuery();
        return rawQuery == null ? rawPath : rawPath + "?" + rawQuery;
    }

    private static String stripIpv6BracketsIfNeeded(String host) {
        // java.net.URI#getHost returns IPv6 without brackets, while the custom class may keep them.
