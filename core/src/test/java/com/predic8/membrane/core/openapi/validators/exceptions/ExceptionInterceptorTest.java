...
public class ExceptionInterceptorTest extends AbstractSecurityValidatorTest {
    ...
    void paramWithNoType() throws Exception {
        ...
        assertEquals("https://membrane-api.io/error/internal/openapi/openapi-parsing/request",json.get("type").asText());
    }
    ...
    void nowhere() throws Exception {
        ...
        assertEquals(500,exc.getResponse().getStatusCode());
        JsonNode json = om.readTree(exc.getResponse().getBodyAsStream());
        assertEquals("https://membrane-api.io/error/internal/openapi/openapi-parsing/response",json.get("type").asText());
    }
