...
public class OAuth2RedirectNormalTest extends OAuth2RedirectTest {

    OAuth2AuthFlowClient getOAuth2AuthFlowClient() {
        return new OAuth2AuthFlowNormalClient(AUTH_SERVER_BASE_URL, CLIENT_BASE_URL);
    }
    ...
    @NotNull SSLableProxy getAuthorizationServerRule() {
        ...
        azureRule.getInterceptors().add(new OAuth2AuthorizationServerInterceptor() {
            ...
            public void init() {
                super.init();
                getWellknownFile().setSupportedResponseModes(Set.of("query", "fragment"));
            }
        });
        return azureRule;
    }
